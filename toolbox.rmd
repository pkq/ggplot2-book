...
1.  What geoms would you use to draw each of the following named plots?
    ...
    1. Scatterplot: [//]: (geom_pont())
    1. Line chart:  [//]: (geom_line())
    1. Histogram:   [//]: (geom_hist())
    1. Bar chart:   [//]: (geom_bar())
    1. Pie chart:   [//]: (geom_bar())

1.  What's the difference between `geom_path()` and `geom_polygon()`?  [//]: (polygon is a filled path)
    What's the difference between `geom_path()` and `geom_line()`?  [//]: (the order in which points are connected)

1.  What low-level geoms are used to draw `geom_smooth()`?   [//]: (geom_line())
    What about `geom_boxplot()` and `geom_violin()`?  [//]: (geom_rect(), geom_poly())
...
1.  Modify the following plot so that you get one boxplot per integer value 
    ...
    ggplot(mpg, aes(displ, cty, group = round(displ))) + 
      geom_boxplot()
    # use round() to bin by engine size
    ```
...
1.  How many bars are in each of the following plots?
    ...
    # one per each value of drv
    ggplot(mpg, aes(drv, fill = hwy, group = hwy)) + 
      geom_bar(color = "white")
    # one per drv/hwy combination
    library(dplyr)  
...
1.  Install the babynames package. It contains data about the popularity of
    ...
    # original (no distinction by gender)
    ggplot(hadley, aes(year, n)) + 
      geom_line()
    # separate lines by sex
    ggplot(hadley, aes(year, n, group = sex)) + 
      geom_line()
    # separately colored lines by sex
    ggplot(hadley, aes(year, n, color = sex)) + 
      geom_line()
    # separate lines & linetypes by sex
    ggplot(hadley, aes(year, n, group = sex)) + 
      geom_line(aes(linetype = sex))
    # separately colored lines & linetypes by sex
    ggplot(hadley, aes(year, n, color = sex)) + 
      geom_line(aes(linetype = sex))
    ```
...
1.  How does the distribution of `price` vary with `clarity`?
    ...
    ggplot(diamonds, aes(price)) +
      geom_freqpoly(aes(colour = clarity), binwidth = 100, na.rm = TRUE)
    
    ggplot(diamonds, aes(price)) +
      geom_histogram(aes(fill = clarity), binwidth = 100, position = "fill", na.rm = TRUE)
    
    ggplot(diamonds, aes(price, fill = clarity, color = clarity)) +
      geom_density(alpha = 0.2, na.rm = TRUE)
    ```
...
1.  Overlay a frequency polygon and density plot of `depth`. What computed
    ...
    ```{r}
    ggplot(diamonds, aes(depth)) +
      geom_freqpoly(aes(binwidth = 1, na.rm = TRUE)) +
      geom_density(aes(y = ..count.., na.rm = TRUE))
    # requires calculated 'count'..' (density * n)
    ```
