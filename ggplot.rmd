...
```{r}
mpg %>% 
  mutate_each(funs(consump = (100 / ((. * 1.60934)/3.78541))), cty, hwy)
```
...
(mpg_model <- mpg %>% 
  group_by(manufacturer, model, trans, drv) %>% 
  summarise(n = n()) %>% 
  summarise(n = n()) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n)))
...
(mpg_model2 <- mpg %>% 
  separate(col = model, into = c("model", "v2", "v3", "v4")) %>% 
  group_by(manufacturer, model, trans, drv) %>% 
  summarise(n = n()) %>% 
  summarise(n = n()) %>% 
  summarise(n = n()) %>% 
...
ggplot(mpg, aes(model, manufacturer)) +
  geom_point()
# Which models belong to which manufacturer. Not really useful. Color code?
```
...
```{r, eval = FALSE}
# mpg dataset, city & highway mileage mapped to x & y, scatter plot.
```
    1. `ggplot(diamonds, aes(carat, price)) + geom_point()`
```{r, eval = FALSE}
# diamonds dataset, carat & price mapped to x & y, scatter plot.
```
    1. `ggplot(economics, aes(date, unemploy)) + geom_line()`
```{r, eval = FALSE}
# economics dataset, date an unemployment mapped to x & y, trend of unemployment over time.
```
    1. `ggplot(mpg, aes(cty)) + geom_histogram()`
```{r, eval = FALSE}
# mpg dataset, city mileage mapped to x, histogram (count) oif mileage groups.
```
...
```{r}
ggplot(mpg, aes(displ, cty, color = year)) + geom_point() # continuous color scale
ggplot(mpg, aes(displ, cty, shape = year)) + geom_point() # cannot map continuous to shape
ggplot(mpg, aes(displ, cty, size = year)) + geom_point()  # smaller values have smaller size

ggplot(mpg, aes(displ, cty, color = class)) + geom_point() # discrete color for each category
ggplot(mpg, aes(displ, cty, shape = class)) + geom_point() # only first six categories are plotted
ggplot(mpg, aes(displ, cty, size = class)) + geom_point()  # using size for a discrete variable is not advised

ggplot(mpg, aes(displ, cty, color = class, size = year)) + geom_point() # both are plotted with separate legends
```
...
```{r}
ggplot(mpg, aes(displ, cty, shape = year)) + geom_point() # error: cannot map continuous variable to shape
ggplot(mpg, aes(displ, cty, shape = trans)) + geom_point() # only first six categories are plotted
```
...
```{r}
ggplot(mpg, aes(drv, cty, color = displ)) + geom_point() # 
# Front wheel drive cars tend to have better fuel economy
ggplot(mpg, aes(class, displ, color = drv)) + geom_point() # 
# front wheel drive tend to be small or midsize cars with smaller engines
# rear wheel drive are mostly 2seaters or subcompacts with larger engines
# 4 wheel drive are mostly pickups and suvs with larger engines
```
...
ggplot(mpg, aes(displ, cty)) + 
  geom_point() + 
  facet_wrap(~cyl)
# cylindars affect displacement, now looks nonlinear
```
...
```{r}
# control whether scales are the same across panels. free to emphasize patterns within panel 
```
...
```{r}
# reorders a categorical variable by the values of a second variable
```
...
```{r}
ggplot(diamonds, aes(price, color = cut)) + geom_freqpoly(binwidth = 50)
# most diamonds are less that $2500, highest freq are ideal, decreasing by cut
```
...
ggplot(mpg, aes(class)) + geom_bar() + geom_bar(aes(weight = displ))
# scales bar by values of a second variable
```
...
ggplot(mpg, aes(trans)) + geom_bar() + facet_wrap(~class)

ggplot(mpg, aes(cyl, trans)) + geom_jitter()
